...
interface IHintHelpers {
    ...
    function getApproxHint(
        ...
        uint256 _numTrials, 
        uint256 _inputRandomSeed) external view 
            returns (
            uint256 hintId, 
            uint256 diff, 
    ...
    function predictOpenTroveUpfrontFee(
        ...
        uint256 _interestRate) external view returns (uint256);


    function predictAdjustInterestRateUpfrontFee(
        uint256 _collIndex, 
        uint256 _troveId, 
        uint256 _newInterestRate) external view returns (uint256);
    ...
    function predictAdjustTroveUpfrontFee(
        uint256 _collIndex, 
        uint256 _troveId, 
        uint256 _debtIncrease) external view returns (uint256);
    ...
    uint256 _collIndex, 
    uint256 _troveId, 
    address _batchAddress) external view returns (uint256);
